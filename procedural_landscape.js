...
function draw_leaves(pos_x, pos_y, radius, dir_sun, ctx){
   // ctx.fillStyle = "rgba("+pixel.data[0]+","+pixel.data[1]+","+(pixel.data[2]*0.5+200)+","+0x80+")"

   var nb_leaves = 25 //due to high number of branches in tress, this number can make the total amount of leaves explose
...
function draw_trunk(startx, starty, angle, level, max_level, length, original_length, rand_numbers, end_branch, local_ctx){
   ...
   if(level >= max_level){
      ...
      if(end_branch == true){
         ...
         draw_leaves(pos_end[0], pos_end[1], original_length/4, [1,-1], local_ctx)
   ...
   var cur_angle = angle;
   draw_trunk(startx, starty, angle, level+1000, max_level, length, original_length, rand_numbers, false, local_ctx);
   ...
   //branch at the top of the trunk
   {
      ...
      draw_trunk(cur_pos[0], cur_pos[1], new_angle, level+1, max_level, length*0.8, original_length, rand_numbers, true, local_ctx);
   }
   ...
   //side_trunk
   {
      ...
      // var new_pos = [cur_pos[0]+length*Math.sin(new_angle), cur_pos[1]+length*Math.cos(new_angle)];
      draw_trunk(cur_pos[0], cur_pos[1], new_angle, level+1, max_level, length*0.6, original_length, rand_numbers, true, local_ctx);
   }
   ...
   //side_trunk
   {
      ...
      var cur_pos = [startx+start_length*Math.sin(angle), starty+start_length*Math.cos(angle)];
      // var new_pos = [cur_pos[0]+length*Math.sin(new_angle), cur_pos[1]+length*Math.cos(new_angle)];
      draw_trunk(cur_pos[0], cur_pos[1], new_angle, level+1, max_level, length*0.5, original_length, rand_numbers, true, local_ctx);
   }
}

function draw_tree(startx, starty, angle, level, max_level, length, local_ctx){
   ...
   draw_trunk(startx, starty, angle, level, max_level, length, length, rand_numbers, true, local_ctx)
}
...
function draw_tree_shadow(startx, starty, length, sun_dir, local_ctx){
   ...
   // var shadow_strength = 0x01
   local_ctx.fillStyle = "rgba("+0x00+","+0x00+","+0x00+","+0.5+")"
   local_ctx.filter = 'blur('+8+'px)';

   local_ctx.beginPath();
   local_ctx.ellipse(startx, starty, length, length/8, 0, 0, 2*Math.PI);
   local_ctx.fill();
...
function draw_mountain(posx, posy, angle_left, angle_right, angle_ridge, height, max_height, local_ctx){
   local_ctx.strokeStyle = "#000000";
   ...
   var strength_left = left_middle_angle/(3.1415)+0.5
   var height_ratio = height/150;
   height_ratio = (height_ratio>1)?1:height_ratio;
   strength_left = height_ratio*strength_left+(1-height_ratio)*0.5;
   var red = Math.floor(0x20*strength_left)
   var green = Math.floor(0xAF*strength_left)
   var blue = Math.floor(0x30*strength_left)
   //var col_left = 0x010101*Math.floor(strength_left*0xFF);
   ...
   var strength_right = right_middle_angle/(3.1415)+0.5
   var height_ratio = height/150;
   height_ratio = (height_ratio>1)?1:height_ratio;
   strength_right = height_ratio*strength_right+(1-height_ratio)*0.5;
   var red = Math.floor(0x20*strength_right)
   var green = Math.floor(0xAF*strength_right)
   var blue = Math.floor(0x30*strength_right)
   //var col_left = 0x010101*Math.floor(strength_left*0xFF);
   var col_left = blue+(green<<8)+(red<<16)
   var col = '#' + (col_left&0xffffff).toString(16).padStart(6, 0)
   local_ctx.fillStyle = col;
   ...
   //////////snowtop left
   var red = Math.floor(0xFF*strength_left)
   var green = Math.floor(0xFF*strength_left)
   var blue = Math.floor(0xFF*strength_left)
   var col_left = blue+(green<<8)+(red<<16)
   var col = '#' + (col_left&0xffffff).toString(16).padStart(6, 0)
   local_ctx.fillStyle = col;
   local_ctx.strokeStyle = local_ctx.fillStyle;
   ...
   var red = Math.floor(0xFF*strength_right)
   var green = Math.floor(0xFF*strength_right)
   var blue = Math.floor(0xFF*strength_right)
   var col_left = blue+(green<<8)+(red<<16)
   var col = '#' + (col_left&0xffffff).toString(16).padStart(6, 0)
   local_ctx.fillStyle = col;
   local_ctx.strokeStyle = local_ctx.fillStyle;
...
function draw_clouds_sine(local_ctx, sun_dir){
   ...
   for (var x = 0; x < size_canvas[0]; x+=15) {
      for (var y = 0; y < 400; y+=15) {
         ...
         if (Math.random() > prob_cloud){
            ...
            local_ctx.beginPath();
            local_ctx.arc(x,y,20, 0, 2*Math.PI);
            local_ctx.fill();
         }
      }
   }
   ...
   for (var x = 0; x < size_canvas[0]; x+=15) {
      for (var y = 0; y < 400; y+=15) {
         if ( (prob_array[x][y]*10-5) > Math.random() ){
            ...
            local_ctx.strokeStyle = "rgba("+(0xFF*cloud_brightness)+","+(0xFF*cloud_brightness)+","+(0xFF*cloud_brightness)+","+0.9+")"
            // local_ctx.strokeStyle = "#FFFFFF"; //white
            local_ctx.fillStyle = local_ctx.strokeStyle;
            local_ctx.beginPath();
            local_ctx.arc(x,y,20*scale, 0, 2*Math.PI);
            local_ctx.fill();
         }
      }
   }
...
function draw_ground(ctx){
   ...
   var noise = improved_perlin_noise_recurs(1600, height*8, 50, 2);
   ...
   for (var x = 0; x < width; x++) {
      for (var y = 0; y < height; y++) {
         ...
         var y_nonlinear = Math.ceil(Math.pow(ratio_y,1.41)*height*8);
...
sun_dir = [1/Math.sqrt(2), -1/Math.sqrt(2)]; // top right
...
for (var i = max_clouds-1; i >= 0; i--){
   ...
   context_layer.fillStyle = 'rgba(0,0,0,0)'; //maxe context transparent (create a layer)
   context_layer.fillRect(0,0,size_canvas[0], size_canvas[1]);
   context_layer.filter = 'blur('+(8+2*i)+'px)';
   // draw_clouds(context_layer, sun_dir);
   var scale = 1/((i+1)); //make clouds smaller (an blurier)
   draw_clouds(context_layer, [0, -1], scale);
   ctx.drawImage(canvas_layer, 0, 0);
}
...
for (var i = 0; i < max_mountains; i++) {
   ...
   draw_mountain(pos_x, pos_y, max_angle, -max_angle, ridge_angle, (farthness+0.75)*100, (1+0.75)*100, context_layer);
...
for (var i = 0; i < max_trees; i++) {
   ...
   draw_tree(pos_x, pos_y, 3.1415, 0, 4, tree_size, context_layer);
